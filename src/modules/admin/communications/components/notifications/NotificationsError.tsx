...
export default function NotificationsError({ message, onRetry }: NotificationsErrorProps) {
  return (
    <div className="flex min-h-[420px] w-full flex-col items-center justify-center gap-4 rounded-[15px] border border-[#EAEAEA] bg-white p-6 text-center">
      ...
      <div className="space-y-1">
        <h3 className="text-[18px] font-bold text-[#101828]">Failed to load notifications</h3>
        <p className="text-[14px] text-[#667085]">{message ?? "Something went wrong while fetching notifications."}</p>
      </div>
      <button
        type="button"
        onClick={onRetry}
